...
{
	sm9_z256_pairing(w1, &(key->DIDA), C1);
	return 1;
}
	...
	const sm9_z256_fp12_t w1, uint8_t *out)
{
	...
	// B1: check C in G1
	sm9_z256_point_to_uncompressed_octets(C1, cbuf);

	// B2: w = e(C, de);
	// sm9_z256_pairing(w, &key->de, C);
	sm9_z256_fp12_pow(w, w1, key->DIDB);
	sm9_z256_fp12_to_bytes(w, wbuf);

	// B3: K = KDF(C || w || ID, klen)
	sm3_kdf_init(&kdf_ctx, sizeof(k));
	sm3_kdf_update(&kdf_ctx, cbuf + 1, 64);
	sm3_kdf_update(&kdf_ctx, wbuf, sizeof(wbuf));
	sm3_kdf_update(&kdf_ctx, (uint8_t *)id, idlen);
	sm3_kdf_finish(&kdf_ctx, k);

	if (mem_is_zero(k, sizeof(k))) {
		error_print();
		return -1;
	}
